...
class _InternalObj(object):
    ...
    @classmethod
    def new(cls, key, mode=None, IV=None, counter=None, segment_size=0):
        if mode is None:
            mode = MODE_ECB
        segment_size = 0
    ...
    def encrypt(self, plaintext):
        ...
        if self.mode == MODE_ECB:
            while i < length:
                result[i:i + self.block_size] = self._impl.encrypt(
            ...
            while i < length:
                ...
                result[i:i + self.block_size] = bytearray(
                                    self._impl.encrypt(bytes(temp))
                                    )
                self.IV = temp[:]
                i += self.block_size
        elif self.mode == MODE_CFB:
            while i < length:
                temp = bytearray(self._impl.encrypt(bytes(self.IV)))
                i += self.segment_size / 8
                for j in range(0, self.segment_size / 8):
                    result[i + j] = plaintext[i + j] ^ temp[j]
                if self.segment_size == self.block_size * 8:
                    self.IV = temp[:]
                elif self.segment_size % 8 == 0:
                    size = self.segment_size / 8
                    self.IV[:self.block_size - size] = \
                        self.IV[size:self.block_size]
                    self.IV[self.block_size - size:self.block_size] = temp[:]
                else:
            ...
            while i < length:
                ...
                self.IV = bytearray(self._impl.encrypt(bytes(self.IV)))
                self.count = 0
            ...
            while i < length:
                ...
                if len(ctr) != self.block_size:
                    raise TypeError("CTR counter function returned "
                                    ...
                                    self.block_size))
                self.IV = bytearray(self._impl.encrypt(bytes(ctr)))
                self.count = 0
        else:
    ...
    def decrypt(self, ciphertext):
        ...
        if self.mode == MODE_ECB:
            while i < length:
                result[i:i + self.block_size] = bytearray(self._impl.decrypt(
            ...
            while i < length:
                ...
                temp = bytearray(self._impl.decrypt(bytes(
            ...
            while i < length:
                temp = bytearray(self._impl.encrypt(self.IV))
                for j in range(0, self.segment_size / 8):
                    result[i + j] = ciphertext[i + j] ^ temp[j]
                if self.segment_size == self.block_size * 8:
                    ...
                    self.IV[:self.block_size - size] = \
                        self.IV[size:self.block_size]
                    self.IV[self.block_size - size:self.block_size] = \
                        result[i:i + size]
                i += self.segment_size / 8
        else:
            raise RuntimeError(("Unknown ciphertext feedback mode {0}; "
